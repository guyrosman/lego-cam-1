...
class SensorConfig:
    ...
    tof_confirm_ms: int = 100  # require change to persist this many ms (high-freq check); 0 = no persistence
...
def load_config(path: str | Path) -> AppConfig:
    ...
    return AppConfig(
        ...
        sensor=SensorConfig(
            ...
            tof_confirm_ms=int(
                _deep_get(raw, ["sensor", "tof_confirm_ms"], SensorConfig().tof_confirm_ms)
            ),
        ),
    )
